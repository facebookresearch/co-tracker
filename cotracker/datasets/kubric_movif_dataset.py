...
class KubricMovifDataset(CoTrackerDataset):
    ...
    def getitem_helper(self, index):
        ...
        if self.sample_vis_1st_frame:
            ...
            # visibile_pts_last_frame_inds = (visibility[self.seq_len-1]).nonzero(as_tuple=False)[:,0]
            visibile_pts_inds = torch.cat(
                (visibile_pts_first_frame_inds, visibile_pts_mid_frame_inds), dim=0
            )
            # , visibile_pts_last_frame_inds
...
# class DynamicReplicaDataset(VideoFlowDataset):
#     def __init__(self,
#                  data_root='/large_experiments/p3/replay/datasets/synthetic/replica_animals/dynamic_replica/dataset/train',
#                  crop_size=(384, 512),
#                  seq_len=48,
#                  traj_per_sample=384,
#                  sample_vis_1st_frame=False,
...
#             sample_vis_1st_frame=sample_vis_1st_frame,
#             use_augs=use_augs)
#         self.resize_lim = [0.5, 1.5] # sample resizes from here
#         self.resize_delta = 0.1
#         self.max_crop_offset = 25
...
#         visibile_pts_first_frame_inds = (visibility[0]).nonzero(as_tuple=False)[:,0]
#         # print('visibile_pts_first_frame_inds',visibile_pts_first_frame_inds.shape)
#         if self.sample_vis_1st_frame:
...
class DynamicReplicaDataset(CoTrackerDataset):
    def __init__(
        self,
        data_root="/large_experiments/p3/replay/datasets/synthetic/replica_animals/dynamic_replica/dataset/train",
        crop_size=(384, 512),
        ...
        self.seq_names = [
            fname
            for fname in os.listdir(data_root)
            if "left" in fname and os.path.isdir(os.path.join(data_root, fname))
        ]
        print("found %d unique videos in %s" % (len(self.seq_names), self.data_root))
        ...
        for seq_name in self.seq_names:
            fnames = [
                f
                for f in os.listdir(os.path.join(data_root, seq_name, "images"))
                if ".png" in f
            ]
            # print(fnames)
            # break
            ...
            for stride in strides:
                # assert stride*seq_len<300
                # print('stride', stride)
                # print('len(fnames)',len(fnames))
                for start_idx in range(0, len(fnames) - stride * seq_len, seq_len):
                    # print(start_idx)
                    subseq_fnames = fnames[
                        start_idx : start_idx + stride * seq_len : stride
                    ]
                    assert len(subseq_fnames) == seq_len
                    subseq_fnames = [
                        os.path.join(self.data_root, seq_name, "images", f)
                        for f in subseq_fnames
                    ]
                    self.subseq_names.append(subseq_fnames)
        print("self.subseq_names", len(self.subseq_names))

    def getitem_helper(self, index):
        gotit = True
        img_paths = self.subseq_names[index]
        rgbs = []
        # for i, img_path in enumerate(img_paths):
        #     rgbs.append(imageio.v2.imread(img_path)[:, :, :3])
        # rgbs = np.stack(rgbs)

        visibility, traj_2d = [], []
        for f in img_paths:
            f_new = f.replace("images", "dynamic_cloud")
            rsplit = f_new.rsplit("/", 1)
            pth_dir = rsplit[0]
            pth_fname = rsplit[1].rsplit("-", 1)[1]
            # pth_paths.append(
            pth_path = os.path.join(pth_dir, "00" + pth_fname.replace(".png", ".pth"))
            # )
            pth_load = torch.load(pth_path)
            rgbs.append(pth_load["img"].numpy())
            traj_2d.append(pth_load["traj_2d"].numpy())
            # print(pth_load['traj_2d'])
            visibility.append(pth_load["verts_inds_vis"].numpy())

        traj_2d = np.stack(traj_2d)[:, :, :2]
        visibility = np.stack(visibility)
        T, N, D = traj_2d.shape

        # subsample trajectories for augmentations
        visible_inds_sampled = torch.randperm(N)[: self.traj_per_sample * 40]
        ...
        # # print(rgbs.shape)
        # visibles = torch.from_numpy(visibility).float()
        # trajs = torch.from_numpy(traj_2d)
        visibility = torch.from_numpy(visibility)
        traj_2d = torch.from_numpy(traj_2d)

        if self.sample_vis_1st_frame:
            visibile_pts_first_frame_inds = visibile_pts_inds = (visibility[0]).nonzero(
                as_tuple=False
            )[:, 0]
        else:
        ...
        # print('visibile_pts_inds',visibile_pts_inds)
        point_inds = torch.randperm(len(visibile_pts_inds))[: self.traj_per_sample]
        # make sure there's at least one point from the first frame
        if (point_inds < len(visibile_pts_first_frame_inds)).sum() == 0:
            gotit = False
        if len(point_inds) < self.traj_per_sample:
            # print('len(point_inds)<self.traj_per_sample')
            gotit = False
    ...
    def __len__(self):
        # return 10
        # return len(self.rgb_paths)
        return len(self.subseq_names)


class KubricDRDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        crop_size=(384, 512),
        seq_len=24,
        ...
        self.kubric_ds = KubricMovifDataset(
            crop_size=crop_size,
            seq_len=seq_len,
            traj_per_sample=traj_per_sample,
            sample_vis_1st_frame=sample_vis_1st_frame,
        ...
        self.dr_ds = DynamicReplicaDataset(
            crop_size=crop_size,
            seq_len=(seq_len // 2) * 3,
            traj_per_sample=traj_per_sample // 2,
            sample_vis_1st_frame=sample_vis_1st_frame,
            use_augs=use_augs,
        )

    def __getitem__(self, i):
        if i < self.kubric_ds.__len__():
            return self.kubric_ds.__getitem__(i)
        else:
            return self.dr_ds.__getitem__(i - self.kubric_ds.__len__())

    def __len__(self):
        return self.kubric_ds.__len__() + self.dr_ds.__len__()
